...
from screens import ScreenManager, BaseScreen
...
class PauseScreen(BaseScreen):
    def __init__(self):
        ...
        renderer = pygame.font.Font("fonts/Roboto-Bold.ttf", 30)
        self.text_pixels = renderer.render("PAUSED", True, colors["black"])
        self.text_rect = self.text_pixels.get_rect()
        self.text_rect.centerx = SCREEN_WIDTH / 2
        self.text_rect.centery = SCREEN_HEIGHT / 2
        self.fullscreen = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT)).convert_alpha()
        self.fullscreen.fill((255,255,255,130))
        self.fullscreen.blit(self.text_pixels, self.text_rect)
    ...
    def draw(self, surf):
        if not self.drawn_once:
            ...
            surf.blit(self.fullscreen, main_surface.get_rect())
...
class PlayScreen(BaseScreen):
    ...
    def update(self):
        ...
        paddle_center = player_1_rect[1] + player_1_rect[3] / 2
        if paddle_center> ball_rect[1]:
    ...
    def draw(self, surf):
        ...
        pygame.draw.rect(surf, colors["white"], game_rect, 1)
        pygame.draw.rect(surf, colors["green"], player_1_rect)
        pygame.draw.rect(surf, colors["green"], player_2_rect)
        pygame.draw.rect(surf, colors["yellow"], ball_rect)
        surf.blit(p1_score_text, p1_score_rect)
        surf.blit(p2_score_text, p2_score_rect)
    ...
    def handle_input(self, evt):
        if evt.type == pl.KEYUP:
            if evt.key == pl.K_m:
                ...
                self.manager.activate("PAUSE")

mgr = ScreenManager()
spause = PauseScreen()
splay = PlayScreen()

spause.set_manager(mgr)
splay.set_manager(mgr)
