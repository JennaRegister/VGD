...
class Maze(object):
    ...
    def generate(self, start=None, end=None):
        ...
        while total_visited_cells < self.rows * self.cols:
            ...
            # this is how animation is done... 'yield' is like 'return' except you put it in a loop, and the next
            # iteration the function resumes from right after the yield
            yield self.start, end, walk
...
# check if this file is being run directly, as opposed to imported
if __name__ == '__main__':
    ...
    def start_generate():
        ...
        e = (random.randint(0,19), random.randint(0,19))
        while e == s:
            e = (random.randint(0,19), random.randint(0,19))
        itr = m.generate(s, e)
    ...
    while not exit:
        ...
        pygame.draw.rect(main_surface, (220,0,0), (e[1]*20,e[0]*20,20,20))
        m.draw(main_surface, (255,255,255), 20, 20)
        pygame.draw.ellipse(main_surface, (0,200,200), (w[1]*20,w[0]*20,20,20))

        pygame.display.update()

        if pygame.key.get_pressed()[pl.K_RIGHT]:
            clock.tick(15)
        else:
            clock.tick(5)
